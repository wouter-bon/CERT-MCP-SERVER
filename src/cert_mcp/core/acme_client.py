...
class ACMEClient:
    ...
    def __init__(
        ...
        self._registration: Optional[messages.RegistrationResource] = None
    ...
    def _get_or_create_account_key(self) -> jose.JWKRSA:
        ...
        if self.account_key_path:
            key_path = Path(self.account_key_path).expanduser()

            if key_path.exists():
                self.logger.info(f"Loading account key from {key_path}")
        ...
        if self.account_key_path:
            ...
            self.logger.info(f"Saved account key to {key_path}")
    ...
    def _get_client(self) -> client.ClientV2:
        ...
        net = client.ClientNetwork(account_key, user_agent="cert-mcp-server/1.0")
    ...
    def get_account_info(self) -> dict:
        ...
        return {
            ...
            "account_uri": self._registration.uri if self._registration else None,
            "account_status": self._registration.body.status if self._registration else None,
        }
