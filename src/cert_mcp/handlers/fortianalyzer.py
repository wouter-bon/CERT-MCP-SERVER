"""FortiAnalyzer device handler using JSON-RPC API."""
...
from ..config.models import FortiAnalyzerDeviceConfig


class FortiAnalyzerHandler(DeviceHandler):
    """Handler for FortiAnalyzer devices using JSON-RPC API."""

    DEVICE_TYPE = "fortianalyzer"

    def __init__(self, device_id: str, config: FortiAnalyzerDeviceConfig):
        """Initialize FortiAnalyzer handler.

        Args:
            device_id: Unique device identifier
            config: FortiAnalyzer device configuration
        """
    ...
    async def _make_request(
        ...
        async with httpx.AsyncClient(verify=self.verify_ssl, timeout=self.timeout) as client:
            ...
            if status.get("code") != 0:
                raise Exception(f"FortiAnalyzer API error: {status.get('message', 'Unknown error')}")
    ...
    async def list_certificates(self) -> List[CertificateInfo]:
        """List certificates on FortiAnalyzer."""
        try:
    ...
    async def get_certificate(self, cert_name: str) -> Optional[CertificateInfo]:
        ...
        try:
            result = await self._make_request(
                "get",
                f"/cli/global/system/certificate/local/{cert_name}"
            )
    ...
    async def import_certificate(
        ...
        """Import a certificate to FortiAnalyzer."""
        try:
            full_cert = cert_pem
            if chain_pem:
    ...
    async def delete_certificate(self, cert_name: str) -> Dict[str, Any]:
        """Delete a certificate from FortiAnalyzer."""
        try:
    ...
    async def replace_certificate(
        ...
        """Replace a certificate on FortiAnalyzer."""
        try:
            full_cert = cert_pem
            if chain_pem:
                if not full_cert.endswith(b"\n"):
    ...
    async def get_device_info(self) -> Dict[str, Any]:
        """Get FortiAnalyzer device information."""
        try:
            result = await self._make_request("get", "/sys/status")
    ...
    def _parse_certificate_data(self, cert_data: dict) -> Optional[CertificateInfo]:
        """Parse FortiAnalyzer certificate data into CertificateInfo."""
        try:
